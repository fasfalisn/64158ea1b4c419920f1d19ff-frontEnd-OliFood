let apiUserApi = new TempApi.UserApi();import TempApi from '../src/index';document.getElementById('i0ydy').onclick = (event) => {
    ...
    event.preventDefault();
    {   location.href= '/CustomerHome' ;}};document.getElementById('iu6xvc').onclick = (event) => {
    event.preventDefault();
    {   location.href= '/NewProduct' ;}};document.getElementById('iz9eo8').onclick = (event) => {
    event.preventDefault();
    {  
     ...
     location.href= '/EditProduct/' + transitionId;}};document.getElementById('i7vj64').onclick = (event) => {
    event.preventDefault();
    {  
      let transitionId = window.location.href.split('/').at(-1);
      let parentId = "";
     ...
     location.href= '/EditProduct/' + transitionId;}};window.onload = () => {let userId = window.location.pathname.replace('/MyProducts/','');apiUserApi.getuser( userId, (error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully. Returned data: ' + data); const map = new Map();try { 
