let apiUserApi = new TempApi.UserApi();import TempApi from '../src/index';document.getElementById('i0ydy').onclick = (event) => {
    ...
    event.preventDefault();
    {   location.href= '/CustomerHome' ;}};document.getElementById('is4aky').onclick = (event) => {
    event.preventDefault();
    {  
     ...
     location.href= '/Products/' + transitionId;}};document.getElementById('i93erj').onclick = (event) => {
    event.preventDefault();
    {  
      let transitionId = window.location.href.split('/').at(-1);
      let parentId = "";
     ...
     location.href= '/Products/' + transitionId;}};window.onload = () => {const filtergetSuppliers = {usercategory: { $eq: "Προμηθευτής"}}; apiUserApi.getByParamsuser( filtergetSuppliers, (error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully. Returned data: ' + data); const subDataElements =[...document.getElementById("ity8k").querySelectorAll( "[dataitem='true']" )].filter(
    (element, index, array) =>
